...
function genSql(){
	...
	echo 'spark.sql("'$drop'")' >> $SQL_FILE
	echo 'spark.sql("'$insert'")' >> $SQL_FILE
...
function initSql(){
	...
	# Mongo表
	echo 'val url="'$url'"'  >> $SQL_FILE
	echo 'spark.sql("create database if not exists' $DB'")' >> $SQL_FILE
	echo 'spark.sql("use perftrace")' >> $SQL_FILE
	echo 'spark.read.format("com.mongodb.spark.sql").options(Map("uri"-> url,"partitioner"->"MongoPaginateBySizePartitioner")).load().registerTempTable("mongo_analysis_day")'  >> $SQL_FILE
	...
	# Hive表
	echo 'spark.sql("create table if not exists' $TO'('$field')' $partition $clustered 'stored as parquet")' >> $SQL_FILE
...
# 执行SQL
function command(){
	cat $SQL_FILE | $APP_HOME/spark/bin/spark-shell --packages org.mongodb.spark:mongo-spark-connector_2.11:2.2.1
}
